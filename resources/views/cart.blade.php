...
@section('content')
  ...
  <section class="container mx-auto flex-grow max-w-[1200px] border-b py-5 lg:flex lg:flex-row lg:py-10">
    ...
    <div class="flex w-full border px-4 py-4">
      ...
      <p class="py-3 text-xl font-bold text-violet-900">Rp{{ $item->product->price }}</p>
      <div class="mt-2 flex w-full items-center justify-between">
      <div class="flex items-center justify-center">
      <button data-id="{{ $item->id }}"
        class="quantity-btn flex h-8 w-8 cursor-pointer items-center justify-center border duration-100 hover:bg-neutral-100 focus:ring-2 focus:ring-gray-500 active:ring-2 active:ring-gray-500"
        data-action="minus">
        -
      </button>
      <div id="qty-{{ $item->id }}"
      ...
      <button data-id="{{ $item->id }}"
        class="quantity-btn flex h-8 w-8 cursor-pointer items-center justify-center border duration-100 hover:bg-neutral-100 focus:ring-2 focus:ring-gray-500 active:ring-2 active:ring-gray-500"
        data-action="plus">
        <!-- jangan di ganti pake logo svg increment atau plus ini   -->
        +
      </button>
      </div>
    ...
    <table class="table-fixed">
      ...
      <tbody>
      @foreach($cartItems as $item)
      @livewire('cart-row', ['cartItem' => $item], key($item->id))
    @endforeach
  ...
  <script>
    ...
    quantityButtons.forEach(button => {
      ...
      try {
        ...
        // totalElem.textContent = `Rp ${data.newTotal}`;

        // const subtotalElem = document.getElementById('subtotal');
        // if (subtotalElem && data.subtotal) {
        //   subtotalElem.textContent = `Rp ${data.subtotal}`;
        // }
